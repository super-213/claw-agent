import { escapeHtml, safeImageSrc } from './format';

const PLACEHOLDER_PREFIX = '\u0000md';
const PLACEHOLDER_SUFFIX = '\u0000';

const isFenceStart = (line: string) => /^\s*(```+|~~~+)/.test(line);
...
const makePlaceholders = () => {
  ...
  return {
    ...
    restore(value: string) {
      return value.replace(/\u0000md(\d+)\u0000/g, (_, index: string) => values[Number(index)] ?? '');
    },
  };
};
...
const isImageMarkdownOnly = (lines: string[]): boolean => {
...
const renderInline = (text: unknown): string => {
  const placeholders = makePlaceholders();
  let value = String(text ?? '');

  value = value.replace(/`([^`]+)`/g, (_, code: string) => placeholders.add(`<code>${escapeHtml(code)}</code>`));

  value = value.replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g, (match, alt: string, src: string) => {
  ...
  value = value.replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g, (match, label: string, href: string) => {
...
const renderParagraph = (lines: string[]) => `<p>${lines.map((line) => renderInline(line)).join('<br>')}</p>`;
...
export const markdownToHtml = (markdown: unknown): string => {
